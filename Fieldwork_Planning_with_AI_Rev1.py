...
def apply_extracted_rates(extracted: dict, replace_existing: bool = False):
    """Apply extracted AI rates to Streamlit session-state tables."""
    ...
    if not fieldwork.empty:
        ...
        if replace_existing:
            st.session_state.rates_df = fieldwork
        else:
            st.session_state.rates_df = normalize_rates_df(pd.concat([st.session_state.rates_df, fieldwork], ignore_index=True))
...
AI_PROGRAM_COLUMNS = [
    ...
    "Depth_m", "Duration_days", "Depends_On", "Start", "Finish",
    "Bar_Color", "Easting", "Northing", "Access_Notes",
    "Planning_Notes", "Traffic_Management", "Accommodation_Notes",
    "Distance_Notes", "Overhead_Spotter", "Services_Risk", "Groundwater_Risk",
]
...
def normalize_ai_program_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in [
        "Task_ID", "Task_Type", "Location_ID", "Investigation_Type", "Depends_On",
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    ...
    "3. AI Preliminary Program",
    "4. Program, Gantt & Test Location Plan",
    "5. Export / Save",
])
...
st.session_state.rates_df = normalize_rates_df(st.session_state.rates_df)
...
with tab1:
    ...
    st.subheader("Subcontractor Rate Schedule Import")
    rate_schedule_files = st.file_uploader(
        ...
        help="Upload driller, excavator, traffic control or laboratory rate schedules. The AI will extract rates into the tables below for review.",
    )
    ...
    with col_ai2:
        replace_extracted_rates = st.checkbox("Replace existing rate tables", value=False)
    with col_ai3:
        extract_button = st.button("Extract Rates with AI")

    if extract_button:
        try:
            with st.spinner("Extracting rates from uploaded schedule..."):
            ...
            st.success("Rates extracted. Please review and edit the tables below before using them.")
    ...
    st.session_state.rates_df = st.data_editor(
        ...
        column_config={
            ...
            "Daily_Rate": st.column_config.NumberColumn("Daily rate", min_value=0.0, help="Used for cost estimate: Duration_days × Daily_Rate."),
        },
    )
    st.session_state.rates_df = normalize_rates_df(st.session_state.rates_df)
